...
if (isset($_GET['exportar'])) {
    ...
    if ($tipo === 'movimientos') {
        ...
        exportarExcel('reporte_movimientos_' . date('Ymd') . '.xlsx', $data, $headers);
    }

    if ($tipo === 'entradas') {
        $query = "SELECT DATE(em.fecha_entrada) as fecha, m.codigo, m.nombre, 
                         mi.cantidad, p.nombre as proveedor, em.numero_lote
        ...
        $data = $stmt->fetchAll(PDO::FETCH_NUM);
        $headers = ['Fecha', 'Código', 'Material', 'Cantidad', 'Proveedor', 'Lote'];
        exportarExcel('reporte_entradas_' . date('Ymd') . '.xlsx', $data, $headers);
    }

    if ($tipo === 'salidas') {
        $query = "SELECT DATE(sm.fecha_salida) as fecha, m.codigo, m.nombre, 
                         mi.cantidad, u.nombre_completo as destino, sm.numero_orden
        ...
        exportarExcel('reporte_salidas_' . date('Ymd') . '.xlsx', $data, $headers);
    }
}

// Obtener datos para mostrar según el tipo de reporte
$fecha_desde = $_GET['fecha_desde'] ?? date('Y-m-01');
$fecha_hasta = $_GET['fecha_hasta'] ?? date('Y-m-d');
...
<div class="row">
    <div class="col-12">
        <div class="content-card">
            <h5 class="mb-3">
                <?php 
                if ($tipo_reporte === 'movimientos') echo 'Movimientos del Período';
                elseif ($tipo_reporte === 'entradas') echo 'Entradas del Período';
                elseif ($tipo_reporte === 'salidas') echo 'Salidas del Período';
                ?>
            </h5>
            <div class="table-responsive">
                <table class="table table-hover table-sm">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="<?php echo ($tipo_reporte === 'movimientos') ? 7 : 5; ?>" class="text-center text-muted py-4">No hay <?php echo $tipo_reporte; ?> en el período seleccionado</td>
                        </tr>
                        <?php else: ?>
                            ...
                            <tr>
                                ...
                                <?php elseif ($tipo_reporte === 'entradas'): ?>
                                    ...
                                    <td><?php echo $mov['proveedor'] ?? 'N/A'; ?></td>
                                    <td><?php echo $mov['numero_lote'] ?? 'N/A'; ?></td>
                                <?php elseif ($tipo_reporte === 'salidas'): ?>
                                    <td>
                                        <span class="badge bg-info"><?php echo $mov['cantidad']; ?></span>
                                    </td>
                                    <td><?php echo $mov['destino'] ?? 'N/A'; ?></td>
                                    <td><?php echo $mov['numero_orden'] ?? 'N/A'; ?></td>
                                <?php endif; ?>
                            </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
