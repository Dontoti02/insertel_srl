...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'crear') {
        ...
        if ($sedeModel->existeCodigo($sedeModel->codigo)) {
            ...
            if ($sedeModel->crear()) {
                registrarActividad($_SESSION['usuario_id'], 'crear_sede', 'sedes', 
        ...
        if ($sedeModel->existeCodigo($sedeModel->codigo, $sedeModel->id)) {
            ...
            if ($sedeModel->actualizar()) {
                registrarActividad($_SESSION['usuario_id'], 'actualizar_sede', 'sedes', 
    ...
    <div class="col-md-8">
        <div class="content-card">
            <form method="POST">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label class="form-label">Responsable (Administrador)</label>
                        <select class="form-select" name="responsable_id">
                            <option value="">Seleccionar administrador...</option>
                            <?php foreach ($administradores as $admin): ?>
                            <option value="<?php echo $admin['id']; ?>" 
                                    <?php echo (isset($sede_actual['responsable_id']) && $sede_actual['responsable_id'] == $admin['id']) ? 'selected' : ''; ?>>
                                <?php echo $admin['nombre_completo']; ?>
                                <?php if ($admin['sede_actual']): ?>
                                - (Actual: <?php echo $admin['sede_actual']; ?>)
                                <?php endif; ?>
                            </option>
                            <?php endforeach; ?>
                        </select>
