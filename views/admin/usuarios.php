...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'crear') {
        ...
        if ($userModel->existeUsername($userModel->username)) {
            ...
            $id = $userModel->crearConSede();
            if ($id) {
                registrarActividad($_SESSION['usuario_id'], 'crear', 'usuarios', "Usuario creado: {$userModel->username}");
    ...
    if ($accion === 'editar') {
        ...
        if ($userModel->id == $_SESSION['usuario_id'] && $userModel->estado == 'inactivo') {
            ...
            if ($userModel->actualizar()) {
                registrarActividad($_SESSION['usuario_id'], 'editar', 'usuarios', "Usuario actualizado ID: {$userModel->id}");
...
$usuarios = $userModel->obtenerTodos($filtros);

// Obtener datos para formularios
$query_roles = "SELECT * FROM roles WHERE id IN (" . ROL_JEFE_ALMACEN . ", " . ROL_ASISTENTE_ALMACEN . ", " . ROL_TECNICO . ") ORDER BY nombre";
